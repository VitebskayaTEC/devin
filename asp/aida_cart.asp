...
<%
	...
	response.write "<div class='cart-header'>" & id & "</div><div id='aida-data' class='cart-overflow'>"

	' Запрос на получение данных из базы и парсинг данных
	dim sql: sql = "SELECT INum, IPage, IDevice, IGroup, IField, IValue FROM Item WHERE ReportID  = (" _
		& "	SELECT TOP(1) ID FROM Report WHERE UPPER(RHost) = UPPER('" & id & "') ORDER BY RDateTime DESC" _
	...
	response.write "</div><table class='cart-menu'><tr><td onclick='reportDelete()'>Удалить из базы<td onclick='cartClose()'>Закрыть</tr></table>"
%>
<script>
