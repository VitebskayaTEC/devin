	...
	public class DevicesController : Controller
	{
		...
		public JsonResult PrintDefect(int Id)
		{
			try
			{
				using (var db = new DevinContext())
				{
					...
					var sheet = book.GetSheet("Сводная");
		...
		public JsonResult PrintReport()
		{
			...
			using (var db = new DevinContext())
			{
				...
				XSSFWorkbook book;
				using (var fs = new FileStream(Server.MapPath(Url.Action("templates", "content") + "\\devin\\report.xlsx"), FileMode.Open, FileAccess.Read))
				{
					book = new XSSFWorkbook(fs);
				}
