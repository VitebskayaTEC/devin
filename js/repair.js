...
function writeoffExport() {
	$("#console").load("/devin/exes/repair/writeoff_export_to_excel.asp?id=" + id.replace("off", "") + "&r=" + Math.random());
}
...
function writeoffPrint() {
	$.get("/devin/exes/repair/writeoff_export_to_excel.asp?id=" + menuId.replace("off", "") + "&r=" + Math.random(), function(data) {
