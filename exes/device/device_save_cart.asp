...
<%
			...
			if instr(lcase(request.form), lcase(rs(i).name)) > 0 then
                ...
                elseif rs(i).name = "LastRepairDate" then
                    ...
                    if (isDate(newValue)) then 
                        ...
                        ' Если старая дата не совпадает с новой
	...
	if sql <> "" then
		...
		sql = log(id, text)
		response.write text
		if isNewID then response.write "<br /><a href='/devin/devices/##" & newIDtext & "'>Обновить страницу, чтобы обновить ID устройства в общем списке</a>"
	else
